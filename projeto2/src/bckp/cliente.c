...
int recv_msg (int socketfd, struct sockaddr_storage *server_info, socklen_t *server_info_len) {
    ...
    do{
        ...
        if (total_bytes_to_recv == -1) {
            ...
            if ((bytes_recv_one_msg = recvfrom(socketfd, buffer, MAXDATASIZE - 1, 0,
...
int main (int argc, char *argv[]) {
    ...
    char buffer[MAXDATASIZE] = {0};
    struct addrinfo hints, *pointer;
    struct sockaddr_storage *server_info;
    socklen_t server_info_size;
    ...
    inet_ntop(pointer->ai_family,
              get_in_addr(pointer->ai_addr),
                           ip_string,
                           sizeof(ip_string));

    server_info_size = sizeof(server_info);
    ...
    while((num_bytes = recv_msg(socketfd, server_info, &server_info_size)) > 0) {
