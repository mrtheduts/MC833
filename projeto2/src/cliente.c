...
int recv_msg (int socketfd, struct sockaddr_storage *server_addr, socklen_t *cout_size) {
...
int main (int argc, char *argv[]) {
    ...
    server_addr = (struct sockaddr_storage *)(pointer->ai_addr);
    cout_size = sizeof(*server_addr);
    ...
    do{
        ...
        sendto(socketfd, buffer, strlen(buffer), 0,
                (struct sockaddr *)server_addr, cout_size);


    } while((num_bytes = recv_msg(socketfd, server_addr, &cout_size)) <= 0);
